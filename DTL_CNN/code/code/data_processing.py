...
def prepare_datasets_returnSourceVal(source_train_path, target_train_path, target_test_path, label_key, validation_split):
    ...
    target_train_features, target_train_labels = parse_csv(target_train_path, label_key)
    # target_train_features_dummies_overall = pd.get_dummies(target_train_features_overall)

    print("target train features")
    print(target_train_features)
    ...
    target_train_dataset = Dataset(df2ids(target_train_features,cui2idx), target_train_labels)

    source_val_size = math.floor(len(source_train_features.index) * validation_split)
    source_train_size = len(source_train_features.index) - source_val_size
    source_train_features_train = source_train_features.loc[0:source_train_size, :]
    source_train_labels_train = source_train_labels.loc[0:source_train_size]
    source_val_features = source_train_features.loc[(source_train_size + 1):, :]
    source_val_labels = source_train_labels.loc[(source_train_size + 1):]

    # wrap features and labels in Dataset objects
    source_train_dataset = Dataset(df2ids(source_train_features_train,cui2idx), source_train_labels_train)
    source_val_dataset = Dataset(df2ids(source_val_features,cui2idx), source_val_labels)

    print("source_train_feature:" + str(source_train_features_train.shape))
