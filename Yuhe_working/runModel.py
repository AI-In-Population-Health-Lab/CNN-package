...
for llm in llms:
    ...
    df= processDataset("/Users/yuhe/Desktop/AIPH/weekly/2024/March/AC_small.xlsx", disease, disease_label, cui_label)
    # df=changeOrder(df)
    ...
    classifier,log=model_running(device, train_loader, val_loader,embedding_matrix, cui2idx, seeds, epoch, in_channels=inchannels, stride=stride, padding=padding,filter_sizes=[2,3,4])
    ...
    result_name=f"best_results/small/AC_best_result_{llm}_bs{batch_size}_e{epoch}_seed{seeds_n}"
    ...
    log_name=f"experimentLog/small/AC_experiment_log_{llm}_bs{batch_size}_e{epoch}_seed{seeds_n}.txt"
